...
void makeMove(struct Player *player, char board[ROW][COL])
{
    ...
    while(valid == FALSE)
    {
        ...
        if( (lengthS == TWO) && (lengthE == TWO) &&
            (isValid(start, end, board, player->playerChar) == TRUE) )
        {
            valid = TRUE;
...
int isDiagonal(char start[THREE], char end[THREE], char playerChar, char board[ROW][COL])
{
    ...
    // start row and col // end row and col
...
{   // start row and col
    ...
    {   // find the jumped checkers location
    int rowInt = (sRow + rowDiff/2);
    int colInt = (sRow + colDiff/2);
...
int isJump(int sRow, int sCol, int eRow, int eCol, char board[ROW][COL], char playerChar)
{
    ...
    if (playerChar == 'B' && rowDiff == -2 && abs(colDiff) == 2)
    {
        int rowInt = sRow + (rowDiff / 2);
        int colInt = sCol + (colDiff / 2);

        if ((board[rowInt][colInt] != SPACE) && (board[rowInt][colInt] != playerChar))
        {
            jump = TRUE;
        }
    }
    else if (playerChar == 'W' && rowDiff == 2 && abs(colDiff) == 2)
    {
        int rowInt = sRow + (rowDiff / 2);
        int colInt = sCol + (colDiff / 2);
